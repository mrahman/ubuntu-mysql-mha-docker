...
MHA_USER=${MHA_USER:-mha}
MHA_PASS=${MHA_PASS:-mhapass}
...
if [[ -d ${MYSQL_DATA_DIR}/mysql ]]; then
    echo "Found an existing volume of MySQL."
    echo "remove already existing volume of MySQL."
    rm -rf ${MYSQL_DATA_DIR}/*
fi

echo "Initialized MySQL volume is detected in ${MYSQL_DATA_DIR}"
echo "Installing MySQL..."
mysqld --initialize-insecure --datadir ${MYSQL_DATA_DIR} || exit 1
touch /tmp/.EMPTY_DB
echo "Done!"
...
if [[ -z ${1} ]]; then
    ...
    if [[ ${MYSQL_MODE} == master ]]; then
        ...
        echo "MySQL MHA configuration for master"
        if [ ! -f /tmp/.MHA_CONFIGURED ]; then
	    mysql -uroot -e "GRANT ALL ON *.* TO '${MHA_USER}'@'%' IDENTIFIED BY '${MHA_PASS}'; FLUSH PRIVILEGES"
            touch /tmp/.MHA_CONFIGURED
        fi
    fi
    ...
    if [[ ${MYSQL_MODE} == slave ]]; then
        ...
        echo "MySQL MHA configuration for slave"
        if [ ! -f /tmp/.MHA_CONFIGURED ]; then
	    mysql -uroot -e "GRANT ALL ON *.* TO '${MHA_USER}'@'%' IDENTIFIED BY '${MHA_PASS}'; FLUSH PRIVILEGES"
            touch /tmp/.MHA_CONFIGURED
        fi
